...
all_unionable_emb = [{} for i in ks]
seen_pairs = []
start_time = time.time()
for table_id1, table_id2 in cursor2.execute("select distinct table_id1, table_id2 from scores;").fetchall():
    for k in range(len(ks)):
        if str(table_id1) not in all_unionable_emb[k]:
            all_unionable_emb[k][str(table_id1)] = []
        if str(table_id2) not in all_unionable_emb[k]:
            all_unionable_emb[k][str(table_id2)] = []
for table_id1, table_id2 in \
    ...
    if (table_id2, table_id1) not in seen_pairs: 
        ...
        for k in ks:
            if len(matched) >= k:
                if str(table_id2) not in all_unionable_emb[ks.index(k)][str(table_id1)]:
                    all_unionable_emb[ks.index(k)][str(table_id1)].append(str(table_id2))
                if str(table_id1) not in all_unionable_emb[ks.index(k)][str(table_id2)]:
                    all_unionable_emb[ks.index(k)][str(table_id2)].append(str(table_id1))
print("--- Execution time: %s seconds ---" % (time.time() - start_time))
print("Number of processed tables is %d." % count)
print("Number of k-unionable tables is %d." % len(all_unionable_emb[0]))
# writing points to json
print("Saving k-unionability info...")
print("Plotting...")
for i in range(len(ks)):
    with open('testdata/' + str(ks[i]) + '_unionable_emb.json', 'w') as fp:
        json.dump(all_unionable_emb[i], fp)
print("Done.")
