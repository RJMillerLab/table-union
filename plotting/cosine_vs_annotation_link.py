...
labels = np.array([0 for _ in range(neg_cosines)] + [1 for _ in range(pos_cosines)])
cosines = np.concatenate([neg_cosines, pos_cosines])
plt.plot(labels, cosines, "o", alpha=0.25, rasterized=True)
plt.xlim(-0.5, 1.5)
plt.tight_layout()
